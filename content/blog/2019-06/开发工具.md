...
GFlags是Google开源的一个命令行flag（区别于参数）库。简单来说就是在使用命令行时，可以根据名称设置参数。Google Glog是Google的一个开源库，用于实现应用级别的日志，相比较log4系列的日志系统，它更加轻巧灵活，而且功能也比较完善。Google Protocol Buffer(简称 Protobuf)是Google公司内部的混合语言数据标准。 <!--more-->
